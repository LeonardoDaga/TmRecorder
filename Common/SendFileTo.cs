...
namespace SendFileTo
{
    ...
    {
        public static void SendPage(string message, string info, string stacktrace, string SwRelease)
        {
            DebugGenMessageBox imb = new DebugGenMessageBox();
            imb.EmailAttachment = info;
            imb.Message = message;
            ...
            try
            {
                body = "----------------------------------------------------------------" +
                    ...
                    "\r\nSpecialFolder.ApplicationData = " + Environment.SpecialFolder.ApplicationData +
                    "\r\nSwRelease = " + SwRelease +
                    "\r\n----------------------------------------------------------------";

                imb.EmailBody = body;

                if (imb.ShowDialog() == DialogResult.Cancel) return;
            }
            catch (Exception ex)
            {
        ...
        public static void Send(Exception e, string info, string stacktrace, string SwRelease)
        {
            string message = "An error occurred. Would you like to send an error report to Led Lennon?\n" +
                          "Send an email to tmrecorder@gmail.com, attaching the comment that follows in the \n" +
                          "body of the message and the Debug file (that you can generate clicking on the 'Save Debug'\n" +
                          "button) as attachment of the email.\n" +
                         "Your cooperation will be very appreciated.";

            Send(message, e, info, stacktrace, SwRelease);
        }

        public static void Send(string message, Exception e, string info, string stacktrace, string SwRelease)
        {
            DebugGenMessageBox imb = new DebugGenMessageBox();
            imb.EmailAttachment = info;
            imb.Message = message;
