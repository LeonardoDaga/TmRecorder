...
{
    public class TDSComparer2 : IComparer<ExtTMDataSet2>
    {
        public int Compare(ExtTMDataSet2 x, ExtTMDataSet2 y)
        {
            if (x == null)
            {
                if (y == null)
    ...
    public class TeamHistory2 : List<ExtTMDataSet2>
    {
        ...
        public ExtTMDataSet2 actualDts = null;
        ...
        private void AddData(Db_TrophyDataSet db_TrophyDataSet, short isReserves)
        {
            ...
            else
            {
                ...
                // Fill ExtTMDataSet2 with Db_TrophyDataSet
        ...
        public new void Add(ExtTMDataSet2 eds)
        {
            sorted = false;
            base.Add(eds);
        }

        public new void Insert(int index, ExtTMDataSet2 eds)
        {
            sorted = false;
        ...
        public void UpdateDirtyPlayers()
        {
            foreach (ExtraDS.GiocatoriRow grow in PlayersDS.Giocatori)
            {
                ...
                // Update full history
                foreach (ExtTMDataSet2 tds in this)
                {
                    ExtTMDataSet2.GiocatoriNSkillRow row = tds.GiocatoriNSkill.FindByPlayerID(grow.PlayerID);
                    ...
                    ExtTMDataSet2.GiocatoriNSkillRow prow = tds.GiocatoriNSkill.FindByPlayerID(grow.PlayerID);
                    ...
                    if (prow != null)
                    {
                        ...
                        {
                            prow.wBorn = TmWeek.GetBornWeekFromAge(tds.Date, 0, grow.Età);
                        }
                ...
                {
                    ExtTMDataSet2 tds = this.actualDts;
        ...
        {
            foreach (ExtTMDataSet2 eds in this)
            {
        ...
        internal void Save(string dataDirectory)
        {
            ...
            foreach (ExtTMDataSet2 ETDS in this)
            {
                if (release == 0.0f)
                {
        ...
        bool sorted = false;
        internal ExtTMDataSet2 LastTeam()
        {
            if (!sorted)
            {
                TDSComparer2 comparer = new TDSComparer2();
        ...
        {
            ExtTMDataSet2 tds = DSfromDate(date);
        ...
        public ExtTMDataSet2.PlayerHistoryDataTable GetPlayerHistory(int playerID)
        {
            ExtTMDataSet2.PlayerHistoryDataTable table = new ExtTMDataSet2.PlayerHistoryDataTable();

            foreach (ExtTMDataSet2 tds in this)
            {
                ExtTMDataSet2.GiocatoriNSkillRow row = null;
                if ((row = tds.GiocatoriNSkill.FindByPlayerID(playerID)) != null)
                {
                    ExtTMDataSet2.PlayerHistoryRow hrow = (ExtTMDataSet2.PlayerHistoryRow)table.NewRow();
        ...
        internal ExtTMDataSet2 DSfromDate(DateTime dt)
        {
            int ix = 0;
            for (; ix < this.Count; ix++)
            {
        ...
        internal void LoadTraining_NewTM2(DateTime dt, string squad, bool quiet = false)
        {
            ...
            try
            {
                ...
                foreach (string playerTr in playersTr)
                {
                    ...
                    ExtTMDataSet2.GiocatoriNSkillRow gnsr = actualDts.GiocatoriNSkill.FindByPlayerID(id);
                    if (gnsr == null)
                    {
                        // It's a GK
                        TrainingDataSet.PortieriRow row = (TrainingDataSet.PortieriRow)trainingDataSet.Portieri.NewRow();
        ...
        private void UpdateAges(TrainingDataSet trainingDataSet)
        {
            foreach (TrainingDataSet.GiocatoriRow gr in trainingDataSet.Giocatori)
            {
                ...
                {
                    egr.wBorn = wBorn;
                    ExtTMDataSet2.GiocatoriNSkillRow ergr = this.actualDts.GiocatoriNSkill.FindByPlayerID(gr.PlayerID);
                    ergr.wBorn = wBorn;
                }
            }
        ...
        internal void LoadTrainingNew_NewTM(DateTime dt, string squad, TrainersSkills trainers)
        {
            ...
            try
            {
                ...
                // Get all rows of the players table
                players = players.Replace("Â¤", "¤");
                gks = gks.Replace("Â¤", "¤");
                trainstr = trainstr.Replace("Â¤", "¤");
                programs = programs.Replace("Â¤", "¤");
        ...
        internal void FillTeamStats(ref TeamStats teamStats)
        {
            ...
            foreach (ExtTMDataSet2 tds in this)
            {
                ...
                foreach (ExtTMDataSet2.GiocatoriNSkillRow row in tds.GiocatoriNSkill)
                {
                    ...
                    if (lasttds != null)
                    {
                        ExtTMDataSet2.GiocatoriNSkillRow lastrow = lasttds.GiocatoriNSkill.FindByPlayerID(row.PlayerID);
                                                ...
                                                TrainingDataSet tds)
        {
            foreach (TrainingDataSet.GiocatoriRow tdsRow in tds.Giocatori)
            {
                ...
                ExtTMDataSet2.GiocatoriNSkillRow gnsrow = actualDts.GiocatoriNSkill.FindByPlayerID(tdsRow.PlayerID);
                if (gnsrow != null)
        ...
        {
            ExtTMDataSet2 eds = DSfromDate(dateTime);
            if (eds == null) return;

            string strToClip = "ID\tNum\tNat\tName\tAge\tFP\tAda\tStr\tSta\tPac\tMar\tTac\tWor\tPos\tPas\tCro\tTec\tHea\tFin\tLon\tSet\tASI\t\r\n";
            foreach (ExtTMDataSet2.GiocatoriNSkillRow gr in eds.GiocatoriNSkill)
            {
                if (gr.FPn != 0)
                    strToClip += gr.ToExcelString() + "\r\n";
            }
        ...
        internal ExtTMDataSet2 DS_BeforeDate(DateTime dt)
        {
            int ix = this.Count - 1;
        ...
        internal void FillTradingList(Trading trading)
        {
            ...
            foreach (ExtTMDataSet2 eds in this)
            {
                ...
                foreach (ExtTMDataSet2.GiocatoriNSkillRow gnr in eds.GiocatoriNSkill)
                {
        ...
        internal void FillPLTrainingTable(PlayerTraining playerTraining, int playerID)
        {
            ...
            try
            {
                foreach (TrainingDataSet tds in TrainingHist)
                {
                    ...
                    ExtTMDataSet2.GiocatoriNSkillRow gsr = this[ix].GiocatoriNSkill.FindByPlayerID(playerID);
        ...
        internal void UpdatePlayerDB(int ID, string item, object value)
        {
            if (item == "Ada")
            {
                ExtTMDataSet2.GiocatoriNSkillRow gsr = actualDts.GiocatoriNSkill.FindByPlayerID(ID);
                if (gsr != null)
                {
